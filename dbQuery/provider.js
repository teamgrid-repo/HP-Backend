...
exports.findProviderByUserId = async (userId) => {
  return provider
    ...
    .select({ name: 1, email: 1, totalAssigned: 1, organization: 1 });
};

exports.findProviderByOrganisation = async (orgId) => {
  return provider
    .find({ organization: orgId })
    .select({ name: 1, email: 1, totalAssigned: 1 });
};

exports.dynamicUpdateProvider = async (fieldName, fieldData, data) => {
  return provider
    .findOneAndUpdate({ [`${fieldName}`]: fieldData }, data)
    .lean();
};

exports.getDynamicProvider = async (fieldName, fieldData = [], field = {}) => {
  return provider
    .find({ [`${fieldName}`]: { $in: fieldData } })
    .select(field)
    .lean()
    .populate({ path: "userId", select: { status: 1 } });
};

exports.getDynamicProviderWithoutPopulate = async (
  fieldName,
  fieldData = [],
  field = {}
) => {
...
exports.filterProviderByPopulate = async (
  data,
  userIdField = {},
  organisationField = {},
  fields = {}
