...
async function createOrUpdatedSL(data) {
  try {
    ...
    return {
      err: false,
      msg: newSearchLink,
      val: successMessages.SEARCHLINK_CREATED,
    };
  } catch (err) {
...
async function getAllSearchLink(currUser) {
  try {
    ...
    if (currUser["subRole"] === "master" || currUser["subRole"] === "manager") {
      ...
      data = await searchLinkOrm.getSearchLinkByData({
        $or: [{ assignedTo: currUser }, { assignedTo: null }],
      });
    }
    const total = [];
    for (let sLink of data) {
      const findTotalOrg = await organisationOrm.getOrganisations({
        sourceOfFinding: sLink["_id"],
      });
...
async function getALlStatisticsDetails() {
  try {
    ...
    const getAllOrganisationByPublish = await organisationOrm.getOrganisations({
      publish: true,
    });
    const data = {};
    data.organisation = {
    ...
    const getALlSearchLinkByStatus = await searchLinkOrm.getSearchLinkByData({
      claimStatus: true,
    });
    data.searchLink = {
    ...
    const getAllProviderByStatus = await providerOrm.getDynamicProvider(
      "approvedStatus",
      status.APPROVED,
      {}
    );
    const getAllProviderByStatusPending = await providerOrm.getDynamicProvider(
      "approvedStatus",
      status.PENDING,
      {}
    );
    ...
    const getAllSiteClaimApproved = await siteClaimOrm.getClaimByEmail({
      status: status.APPROVED,
    });
    const getAllSiteClaimPending = await siteClaimOrm.getClaimByEmail({
    ...
    for (let appointment of appointmentByState) {
      if (!isEmpty(appointment["siteId"])) {
        ...
        if (!isEmpty(site.state) && !isEmpty(site.state[0])) {
          ...
          if (site["virtual"])
            stateObj["virtual"][`${appointment["status"]}`]++;
          else {
            map(states, (s) => {
              if (!isEmpty(stateObj[s])) {
                ...
                stateObj[s] = {
                  pending: appointment["status"] === status.PENDING ? 1 : 0,
                  approved: appointment["status"] === status.APPROVED ? 1 : 0,
                  cancelled: appointment["status"] === status.CANCELLED ? 1 : 0,
                };
            });
          }
        } else {
...
async function publishAndUnPublish() {
  try {
    const org = await organisationOrm.filterOutOrganisationByRegex({
      state: { $ne: null },
    });
    ...
    const publishSite = await siteOrm.findSites({
      organisationId: { $in: orgPublishIds },
      state: { $elemMatch: { $exists: true } },
    });
    const UnPublishSite = await siteOrm.findSites({
      organisationId: { $in: orgUnPublishIds },
      state: { $elemMatch: { $exists: true } },
    });
        ...
        if (!isEmpty(states)) {
          map(states, (state) => {
            if (!isEmpty(totalSiteClaim[state])) {
              ...
              totalSiteClaim[state] = {
                pending: claimStatus === status.PENDING ? 1 : 0,
                approved: claimStatus === status.APPROVED ? 1 : 0,
                cancelled: claimStatus === status.CANCELLED ? 1 : 0,
              };
            }
          });
        }
    ...
    const publishService = await siteSubcategoryOrm.findDynamicSubcategory(
      "siteId",
      { $in: publishSiteIds }
    );
    const unPublishService = await siteSubcategoryOrm.findDynamicSubcategory(
      "siteId",
      { $in: unPublishSiteIds }
    );
    ...
    for (let service of Object.keys(groupBySubCategory)) {
      ...
      const groupByStateOrg = groupBy(
        subCatData,
        "siteId.organisationId.state"
      );
      let stateCount = {};
    ...
    const groupBySubCategoryPublished = groupBy(
      publishService,
      "subCategoryId.name"
    );
    ...
    return {
      organisationAndSite: {
        totalPublishOrgByState,
        totalUnPublishOrgByState,
        totalPublishSiteByState,
        totalUnPublishSiteByState,
        totalUnPublishServiceByState,
        totalPublishServiceByState,
        totalSiteClaim,
      },
    };
  } catch (err) {
...
async function providerDataStatistics() {
  try {
    ...
    const getAllSubCategory =
      await siteSubcategoryOrm.filterDataOfSiteSubCategory({});
    ...
    for (let user of users) {
      ...
      if (user["role"] === identity.ADMIN && user["subRole"] === "analyst")
        analyst++;
      if (user["role"] === identity.ADMIN && user["subRole"] === "manager")
        manager++;
    }
...
async function overAllStatisticsSearchLink() {
  try {
    ...
    let totalByState = {};
    for (let state of Object.keys(groupByState)) {
      totalByState[state] = groupByState[state].length;
    }
    let unclaimedByState = filter(allSearchLink, { claimStatus: false });
    const unClaimed = groupBy(unclaimedByState, "states");
    let unClaimByState = {};
    for (let claim of Object.keys(unClaimed)) {
      unClaimByState[claim] = unClaimed[claim].length;
    }
    ...
    const inProgress = await searchLinkOrm.getSearchLinkByData({
      close: false,
    });
    ...
    let providerList = {};
    for (let listingCount of Object.keys(groupByStateListProvider)) {
      providerList[listingCount] =
        groupByStateListProvider[listingCount].length;
    }
    ...
    return {
      ...
      msg: {
        ...
        ...appointment.msg,
        message: {
          room: totalChatRoom["length"],
          totalMsg: totalMessage["length"],
        },
        providerList,
      },
    };
