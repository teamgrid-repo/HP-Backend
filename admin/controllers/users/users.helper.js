...
async function assignRoleUserToUser(user, currUser) {
  try {
    if (!isEmpty(user["assigningId"]))
      return { err: true, msg: "This Email is already assigned to one Admin" };
    const id = user["_id"];
...
async function updateAdminUser(id, data) {
  try {
    ...
    if (
      !isEmpty(findUser["assigningId"]) &&
      (!isEmpty(data["providerEmail"]) || data["remove"])
    ) {
      const providerUser = await userOrm.findUser({
        _id: findUser["assigningId"],
      });
      if (isEmpty(providerUser))
        return { err: true, msg: "User Email not find" };
      data.assigningId = null;
      await userOrm.updateUserById(findUser["assigningId"], {
        assigningId: null,
      });
    }
    ...
    if (!isEmpty(data["providerEmail"])) {
      ...
      if (isEmpty(user) || user["role"] === identity.GENERAL_USER)
        return { err: true, msg: "User Email not Valid" };
...
async function freezeAccount(data) {
  try {
    ...
    if (isEmpty(userId))
      return { err: true, msg: errorMessages.SOMETHING_WENT_WRONG };
...
async function deleteEverything(userId) {
  try {
    ...
    //SAVED SEARCHES
    const savedSearched = await saveSearchesHelper.deleteSaveSearchedByUserId(
      id
    );
    ...
    if (findId["role"] === identity.PROVIDER) {
      const providerDetails = await providerOrm.filterProvider({"userId":id});
    ...
    const usss= await sitesSubcategoryOrm.addstaticpoc(
      ...
      const siteSubCategory =
        await siteSubCategoryHelper.deleteSiteSubcategoryByUserId(id);
      ...
      if (!isEmpty(providerProfile) && providerProfile.err)
...
async function deleteAccount(data) {
  try {
    ...
    console.log("dataaaaa in delete",data)
    const { id } = data;
    let deleteAllData = { err: true, msg: errorMessages.SOMETHING_WENT_WRONG };
    let typeAccount = data["accountType"];
    if (
      typeAccount === providerType.SYSTEM_PROVIDER ||
      typeAccount === providerType.NEW_REGISTER_PROVIDER ||
      typeAccount === providerType.GENERAL_USER ||
      typeAccount === providerType.CLAIM_PROVIDER
    ) {
      let findId;
      ...
      if (typeAccount === providerType.CLAIM_PROVIDER) {
        const findClaimSite = await claimProviderOrm.getOneSiteClaim({ _id: id });
        if (!isEmpty(findClaimSite)) {
