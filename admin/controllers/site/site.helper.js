...
} = require("../../../controllers/sitesSubCategory/sitesSubCategory.helper");
...
async function allocateSiteSubCategoryWithSite(siteId) {
  try {
    ...
    for (let subCategory of subCategories) {
      const checkIsAlreadyExit = await siteSubcategoryOrm.findSiteSubcategory({
...
async function createSties(data) {
  try {
    const findAlreadyExistSites = await siteOrm.findSites({
      organisationId: data.organisationId,
      name: data.name,
    });
    if (!isEmpty(findAlreadyExistSites))
      return {
        err: true,
        msg: "Site Name is already associate with this organisation",
      };
    ...
    if (!isEmpty(siteSubCategory) && siteSubCategory.err)
...
async function getSiteDetails(organisationId) {
  try {
    const getAllSites = await siteOrm.findSites({
      organisationId: organisationId,
    });
    const dataValue = [];
    const siteApproval = await dataApprovalOrm.getSiteDataApproval({
      organisationId,
      status: status.PENDING,
    });
    for (let site of getAllSites) {
      const siteInfo = await getSiteAndSiteSubCategory(site, site["_id"]);
      const getAllPendingSite = find(siteApproval, {
        method: method.UPDATE,
        siteId: site["_id"],
      });
      dataValue.push({
        approvalPending: !isEmpty(getAllPendingSite),
        approval: getAllPendingSite,
        ...siteInfo.msg,
      });
    }
    if (!isEmpty(dataValue)) {
      return {
        err: false,
        msg: [...dataValue, ...filter(siteApproval, { method: method.CREATE })],
...
async function getAllSitesByUser(data) {
  try {
    const findProvider = await ProviderOrm.getDynamicProvider(
      "userId",
      data.userId,
      {
    ...
    if (isEmpty(findProvider))
      return { err: true, msg: "Provider Data not found" };
    if (!isEmpty(findProvider) && !findProvider[0].makeAccountPrimary)
      return { err: true, msg: "account is not primary" };
...
async function getAllSiteByOrganisation(organisationId) {
  try {
    const checkValidOrganisation = await organisationOrm.getOrganisationById(
      organisationId
    );
    if (isEmpty(checkValidOrganisation))
...
async function findAllState() {
  try {
    ...
    for (let state of findStates) {
      const getAddressWiseState = await siteOrm.findSitesByFields(
        { state: { $in: state } },
        {}
      );
...
async function updatedSiteDataAndSiteSubCategory(siteId, data) {
  try {
    ...
    for (let newS of newSubCategory) {
      const findS = find(
        oldSubCategory,
        (data) => data.toString() === newS.toString()
      );
    ...
    for (let oldS of oldSubCategory) {
      const findS = find(
        newSubCategory,
        (data) => data.toString() === oldS.toString()
      );
...
async function deleteSiteBySiteId(siteId) {
  try {
    ...
    const findSiteSubCategory = await siteSubcategoryOrm.findDynamicSubcategory(
      "siteId",
      siteDetails._id,
      { _id: 1 }
    );
    ...
    const findInstanceOfSiteListingItems =
      await saveListingOrm.getDynamicSavedItemListing({ siteId }, { _id: 1 });
