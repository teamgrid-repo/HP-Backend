...
async function sitesSubCategoryCreated(data, userId, localUserRole) {
  try {
    return dataApprovalHelper.sitesSubCategoryCreated(
      data,
      userId,
      localUserRole
    );
...
async function getSiteAndSiteSubCategory(getSiteDetails, siteId) {
  try {
    if (isEmpty(getSiteDetails)) {
      return { err: true, msg: "SiteId is not valid" };
    }
    const findExitingSiteSubcategory =
      await siteSubCategoryOrm.findSiteSubCategoryBySiteId({ siteId });
    const data = [];
    for (let siteSubcategory of findExitingSiteSubcategory) {
      const specialQualification =
        await specialQualifOrm.dynamicSpecialQualification(
          { _id: { $in: siteSubcategory["specialQualif"] } },
          { createdAt: 0, updatedAt: 0, __v: 0 }
        );
      const findDataApprovalByService =
        await dataApprovalOrm.getSiteServiceDataApproval({
          siteId: siteSubcategory["siteId"],
          subCategoryId: siteSubcategory["subCategoryId"],
          status: status.PENDING,
        });
      data.push({
        ...siteSubcategory["_doc"],
        specialQualif: Object.keys(groupBy(specialQualification, "name")),
        approvalPending: !isEmpty(findDataApprovalByService),
      });
    }
    return {
      err: false,
      msg: { ...getSiteDetails, siteSubCategoryInfo: data },
    };
  } catch (err) {
    return { err: true, msg: err };
  }
...
async function getSiteSubCategory(data, accountOwnerUserId) {
  try {
    const checkProviderAccount = await providerOrm.getDynamicProvider(
      "userId",
      accountOwnerUserId
    );
    ...
    const findOrganisation = await organisationOrm.fetchAllInfoByOrganisationId(
      [organisationId]
    );
    for (let organisation of findOrganisation) {
      ...
      if (!isEmpty(siteInfo)) {
        for (let site of siteInfo) {
          const siteSubCategory = await getSiteAndSiteSubCategory(
            site,
            site._id
          );
  ...
  try {
    ...
    if (!isEmpty(siteSubcategories)) {
      for (let siteSubcategory of siteSubcategories) {
        ...
        await siteSubCategoryOrm.updateSiteCategory(id, {
          $pull: { poc: { $in: userId } },
        });
      }
...
async function getAllSiteSubcategoryByUserId(userId) {
  try {
    ...
    for (let site of allSites) {
      ...
      const categoryInfo = await categoryController.getCategoryOfCure(
        categoryArr,
        Subcategories
      );
      siteData.push({
        ...site,
        siteSubCategory: categoryInfo,
      });
    }
...
async function searchByKeyWord(keyWord = "", data = []) {
  try {
    ...
    const getAllOrganisation =
      await organisationOrm.filterOutOrganisationByRegexBYData(
        {
          $or: [
    ...
    for (let organisation of getAllOrganisation) {
      if (!isEmpty(organisation["sitesInfo"])) {
        siteIds = map(organisation["sitesInfo"], (org) => org._id);
      }
    }
    ...
    if (!isEmpty(siteIds)) {
      filterData = [...filterData, { siteId: { $in: siteIds } }];
    }

    const specialQualification = await specialQualificationOrm.filterOutByRegex(
      "name",
      keyWord
    );
    if (!isEmpty(specialQualification)) {
      ...
      filterData = [
        ...filterData,
        { specialQualif: { $in: Object.keys(groupBySpecialIds) } },
      ];
    }
    let userName = await providerOrm.filterProvider({
      $or: [
        { name: { $regex: keyWord, $options: "i" } },
    ...
    if (!isEmpty(userName)) {
      filterData = [
        ...filterData,
        { poc: { $in: Object.keys(groupBy(userName, "userId")) } },
      ];
    }
    if (!isEmpty(filterData)) {
      const filterOutResult = await dynamicFetchingOfData(data, {
        $or: filterData,
      });
      if (!isEmpty(filterOutResult) && filterOutResult.err)
...
async function orderBySiteId(groupBySiteId) {
  try {
    let dataValue = [];
    let storeCategory = {};
    //console.log("groupBySiteId", groupBySiteId);
    for (let site of groupBySiteId) {
      ...
      for (let service of siteSubCategories) {
        ...
        if (isEmpty(storeCategory[categoryId])) {
          storeCategory[categoryId] = service.subCategoryId.category_id;
        }
      }
      //console.log("siteDetails", siteDetails["_doc"]);
      dataValue.push({
        ...siteDetails["_doc"],
        categoryInfo: map(Object.keys(cat), (ids) => {
...
async function storeSiteSubCategoryInSession() {
  ...
  time = moment().utc();
  const allOrg = await organisationOrm.getOrganisations({ publish: true });
  const allSite = await siteOrm.findSites({
    organisationId: Object.keys(groupBy(allOrg, "_id")),
  });
  siteSubcategoryData = await siteSubCategoryOrm.filterDataOfSiteSubCategory({
    siteId: Object.keys(groupBy(allSite, "_id")),
  });
...
async function filterSiteProvider(filterData) {
  try {
    ...
    if (!isEmpty(filterData)) {
      ...
      if (
        Object.keys(filterData).includes("leaf") &&
        filterData.leaf === true
      ) {
        const { leaf } = filterData;
        let applicable, notApplicable;
        if (!isEmpty(filterData.category)) {
          applicable = await cureCategoryOrm.getAllSubCategory({
            _id: { $in: filterData.category },
            applicable: true,
          });
          notApplicable = await cureCategoryOrm.getAllSubCategory({
            _id: { $in: filterData.category },
            applicable: false,
          });
        } else {
        ...
        const filterLeaf = await dynamicFetchingOfData(data, {
          $or: [
            {
              subCategoryId: {
                $in: Object.keys(groupBy(notApplicable, "_id")),
              },
            },
            {
              subCategoryId: { $in: Object.keys(groupBy(applicable, "_id")) },
              leaf,
            },
          ],
        });
      ...
      if (
        ...
        filterData.additionalResource === true
      ) {
        ...
        data = await fetchResult(data, {
          siteId: { $in: Object.keys(groupBy(siteData, "_id")) },
        });
        if (isEmpty(data)) return { err: false, msg: [] };
      }
        ...
        if (!isEmpty(filterData.category)) {
          ...
          for (let subId of filterData.category) {
            ...
            if (!isEmpty(findSpQ)) {
              ...
              for (let sPId of findSpQ) {
                const id = sPId["_id"];
                if (specialQualification.includes(id.toString()))
                  applied = true;
              }
              if (applied) {
                applicable.push(subId);
          ...
          const filterSpQ = await dynamicFetchingOfData(data, {
            $or: [
              ...
              {
                subCategoryId: { $in: applicable },
                specialQualif: { $in: specialQualification },
              },
            ],
          });
      ...
      if (!isEmpty(filterData.keywords)) {
        ...
        if (!isEmpty(keyWordsData) && keyWordsData.err) {
          return keyWordsData;
        }
      ...
      if (!isEmpty(filterData.states)) {
        ...
        const findSiteByState = await siteOrm.findSites({
          state: { $in: states },
        });
        data = await fetchResult(data, {
          siteId: { $in: Object.keys(groupBy(findSiteByState, "_id")) },
        });
        if (isEmpty(data)) return { err: false, msg: [] };
      }
      if (isEmpty(data)) return { err: false, msg: [] };
    ...
    if (
      ...
      console.log("insideeeeeeeeeeeeeeeeeeeeeeeeeeeee falseeeeeeeeeeeeeeeeeeeee")
      const { additionalResource } = filterData;
      console.log("additionalResource",additionalResource)
      const siteData = await siteOrm.findSites({
        additional: true,
      });
      data = await fetchResult(data, {
...
async function getPocAllData(poc = []) {
  try {
    let data = [];
    if (!isEmpty(poc)) {
      const getAllData = await providerOrm.getDynamicProvider("userId", poc, {
        name: 1,
        email: 1,
        contact: 1,
        userId: 1,
...
async function findSiteSubCategoryAlongWithOrg(siteSubCategories) {
  try {
    ...
    const getAllSubCategories = await cureCategoryOrm.getSubcategoryById(
      Object.keys(groupBySubCategoryId)
    );
    let totalSiteContact = 0;
    for (let siteSubCategory of siteSubCategories) {
      const data = pick(siteSubCategory, [
        "leaf",
        "isHippa",
        "poc",
        "price",
      ...
      const findSubCategoryData = find(getAllSubCategories, {
        _id: siteSubCategory.subCategoryId,
      });
...
async function getAllSteteLocationsByCategories() {
  try {
    ...
    for (let category of cureCategory) {
      ...
      const cureSubCategory = await cureCategoryOrm.findDistinctCureSubcategory(
        category["_id"]
      );
      if (isEmpty(cureSubCategory))
        return { err: true, msg: "No Category found" };
      const subCategory = await cureCategoryOrm.getSubcategoryByCategoryId(
        category["_id"],
        { name: 1, active: 1 }
      );
      const allDistinctSites = await siteSubCategoryOrm.getDistinctSites(
        "siteId",
        {
      ...
      const siteLocationData = await siteOrm.findSitesByFields(
        { _id: { $in: allDistinctSites } },
        { location: 1 }
      );
...
async function allFilterProvider(filterData) {
  try {
    ...
    const inOffice = await siteOrm.findSitesCount({
      _id: { $in: totalResult },
      location: { $ne: null },
      virtual: false,
      homeVisit: false,
    });
    const virtualSite = await siteOrm.findSitesCount({
      _id: { $in: totalResult },
      virtual: true,
    });
    const homeVisit = await siteOrm.findSitesCount({
      _id: { $in: totalResult },
      homeVisit: true,
      location: { $ne: null },
    });
    return {
      err: false,
      msg: {
        total: inOffice + virtualSite + homeVisit,
        virtualSite,
        homeVisit,
        inOffice,
      },
