...
module.exports.getAllSiteSubcategory = async (req, res) => {
  try {
    ...
    if (req.user.role !== "provider")
      return errorResponse(req, res, "You are not a provider");
    const getAllSitesSubCategoryDetails =
      await siteSubcategoryHelper.getSiteSubCategory(param, userId);
    if (
      !isEmpty(getAllSitesSubCategoryDetails) &&
      getAllSitesSubCategoryDetails.err
    ) {
    ...
    return successResponse(
      req,
      res,
      getAllSitesSubCategoryDetails.msg,
...
module.exports.filterSitesProvider = async (req, res) => {
  try {
    ...
    if (!isEmpty(getProvider) && !getProvider.err) {
      ...
      const result = await siteSubcategoryHelper.allFilterProvider(
        filterProvider
      );
      filterProvider = Object.entries(groupBy(filterProvider, "siteId._id"));
      if (isEmpty(param.address)) {
        filterProvider =
          result.msg.total > 100
            ? filterProvider.slice(index, index + 100)
            : filterProvider;
      }
      const provider = await siteSubcategoryHelper.orderBySiteId(
        filterProvider
      );
      // provider.msg.sort(function (a, b) {
      //   // sort in an ascending order
      //   if (a.name < b.name) {
      //     return -1;
      //   }
      ...
      return successResponse(
        req,
        res,
        { count: result.msg, provider: provider.msg },
        successMessages.DATA_FETCHED
      );
    } else {
...
module.exports.findAllStateLocationByCategory = async (req, res) => {
  try {
    const stateLocation =
      await siteSubcategoryHelper.getAllSteteLocationsByCategories();
