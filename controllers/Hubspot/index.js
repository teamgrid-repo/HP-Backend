    ...
    try {
        ...
        if(!isEmpty(category) && !isEmpty(data.subcategory)){
            for(let cat of category){
                ...
                categoryS += `${cureCategoriesSameAsHubspot(findCategory["weight"])};`
            }
            for(let subCat of cureSubCategory){
                subCategoryS += `${cureSubCategoriesSameAsHubspot(subCat["originalName"])};`
            }
        }
        ...
        let priceFilter= ''
        if(!isEmpty(data.price)){
            for(let pr of data.price) priceFilter += `${pr};`
        }
