...
async function providerTakenBehalfAppointment(
  currUserId,
  siteId,
  subCategoryId,
  sendEmail,
  ...
  try {
    const subCategory = await siteSubcategoryOrm.filterDataOfSiteSubCategory({
      siteId,
      subCategoryId,
    });
    ...
    if (isEmpty(findSubCategory["poc"])) {
      ...
      const providers = await providerOrm.filterProviderByfields(
        { organization: orgId, makeAccountPrimary: true },
        { createdAt: 0, updatedAt: 0, __v: 0 }
      );
      for (let provider of providers) {
        if (!isEmpty(provider["email"]) && provider["EmailMessage"]) {
          email.push(provider["email"]);
        }
        if (!isEmpty(provider["contact"]) && provider["textMessage"]) {
          contact.push(provider["contact"]);
        }
      }
    } else {
      //   const poc = findSubCategory["poc"];
      //   const providers = await providerOrm.filterProviderByfields(
      //     { userId: Object.keys(groupBy(poc, "_id")) },
      //     { createdAt: 0, updatedAt: 0, __v: 0 }
      //   );
      //   for (let provider of providers) {
      //     if (!isEmpty(provider["email"]) && provider["EmailMessage"]) {
    ...
    if (!isEmpty(contact)) {
      for (let number of contact) {
        ...
        sendSmsFunction(
          number,
          `You have an appointment request waiting for you in your Her PLAN appointment portal. Please click here ${messageLink.APPOINTMENTS} to go to your messages to view and respond.`
        );
      }
    }
...
async function createAppointment(data) {
  try {
    ...
    if (
      !isEmpty(findAlreadyBookedAppointment) &&
      findAlreadyBookedAppointment[0].status !== status.CANCELLED
    )
      return {
        err: true,
        msg: "You have already requested an appointment for that date.",
      };
    const format = {
      clientId,
      providerId,
    ...
    if (role === "provider") {
      ...
      if (!isEmpty(clientUser) && !clientUser["optShareData"])
        return { err: true, msg: "Client are not allowed to Share his Data" };
      format.referredBy = data.userId;
      const createUniqueRoom = generateVerifyCode("Aa0");
      const roomName = createUniqueRoom.code;
      const getOrCreateRoom = await messageHelper.createOrGetRoomByRoomId(
        roomName,
        [clientId, providerId],
        {
          group: true,
      ...
      if (!isEmpty(getOrCreateRoom) && getOrCreateRoom.err)
    ...
    await providerTakenBehalfAppointment(
      data.userId,
      siteId,
      subCategoryId,
      data.email,
    ...
    return {
      err: false,
      msg: createNewAppointment,
      val: "Appointment Request has been sent successfully",
    };
  } catch (err) {
    return { err: true, msg: err };
  }
...
async function getAllAppointment(data) {
  try {
    ...
    if (role === "user") {
      ...
      if (isEmpty(findGeneralUser))
        return { err: true, msg: errorMessages.USER_NOT_EXIST };
      dataValue = await appointmentOrm.getAppointmentByPopulate({
        clientId: userId,
      });
    } else if (role === "provider") {
      const getProviderDetails = await providerOrm.filterProvider({ userId });
      console.log("getProviderDetails",getProviderDetails)
      if (isEmpty(getProviderDetails))
        return { err: true, msg: errorMessages.USER_NOT_EXIST };
       ...
       */
      const getAllActionAppointment =
        await appointmentOrm.getAppointmentByPopulate({ providerId: userId });
      ...
      //   if (primaryAccount) {
      //     const siteIds = await siteOrm.findSites({
      ...
      //       );
      //  }
        // console.log("findSiteWithNoPoc",findSiteWithNoPoc)
        // for (let site of findSiteWithNoPoc) {
        //   console.log("site",site)
        //   const findPendingAppointment =
        //     await appointmentOrm.getAppointmentByPopulate({
        //       _id: { $nin: Object.keys(groupBy(getAllActionAppointment, "_id")) },
        //       siteId: site["siteId"],
        //       subCategoryId: site["subCategoryId"],
        //       status: status.PENDING,
        //     });
        //     console.log("findPendingAppointment",findPendingAppointment)
    ...
    for (let appoint of dataValue) {
      ...
      const clientData = pick(appoint.clientId, [
        "role",
        "name",
        "email",
        "_id",
      ...
      const canceledByData = !isEmpty(appoint.canceledBy)
        ? [appoint.canceledBy]
        : [];
      const service = await siteSubcategoryOrm.findSiteSubCategoryBySiteId(
        {
          siteId: appoint["siteId"],
...
async function updateAppointment(data, appointmentId) {
  try {
    let email = "";
    let contact = "";
    const findAppointment = await appointmentOrm.getAppointment({
      _id: appointmentId,
    });
    if (isEmpty(findAppointment))
      return { err: true, msg: "Appointment is not valid" };
    if (isEmpty(data.status))
      return { err: true, msg: "status Parameter is mandatory" };
    if (data.status === status.APPROVED || data.status === status.CANCELLED) {
      ...
      if (currDate > date)
        return {
          err: true,
          msg: `${data.status} action can't be performed to previous date`,
        };
      ...
      if (!isEmpty(client)) {
        ...
        contact =
          client["profileId"].textMessage &&
          !isEmpty(client["profileId"].contact.toString())
            ? client["profileId"].contact.toString()
            : "";
      }
    } else {
    ...
    if (!isEmpty(contact)) {
      sendSmsFunction(
        contact,
        `A provider has responded to your appointment request. Click here ${messageLink.APPOINTMENTS} and go to your appointments to view and respond.`
      );
    }
    return {
      err: false,
      msg: [],
      val: "Your Response was successfully received",
    };
  } catch (err) {
    return { err: true, msg: err };
  }
