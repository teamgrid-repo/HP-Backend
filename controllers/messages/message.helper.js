...
async function createChat(roomDetails, users, dataValue){
    try {
        ...
        let getRoomAnotherUserByRoomName1 = await messageOrm.getRoomByData({ room: Object.keys(groupAllChatRoom), userId: { $ne: dataValue.localUserId}});

        let getRoomAnotherUserByRoomName  = getRoomAnotherUserByRoomName1.filter(function (el) {
