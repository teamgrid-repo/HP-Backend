...
async function createDataApprovalForSubUser(data) {
  try {
    const findEmail = await dataApprovalOrm.getDataApprovalSubUser({
      email: data.email,
    });
    if (!isEmpty(findEmail)) {
      ...
      const findEmailUser = await dataApprovalOrm.getDataApprovalSubUser({
        email: data.email,
        status: status.PENDING,
      });
      console.log("findEmailUser",findEmailUser)
      if (!isEmpty(findEmailUser)) {
        return {
          err: true,
          msg: "Already One Action Remaining By Admin on Same Email",
        };
      }
    }
    await dataApprovalOrm.createDataSubUser({
      ...data,
      status: status.PENDING,
    });
...
async function updateDataApprovalRecord(data) {
  try {
    ...
    if (data.status === status.APPROVED && user.method === method.CREATE) {
      ...
      //   const template = content([
      //     "You have been approved as a provider user in the Her PLAN directory! Click here to login. You can now save lists and searches of providers, save custom quiz results, and directly message other providers on the platform. Please be sure to turn on appointments and messages.",
      //   ]);
