...
const {
  ...
  identity,
  leadStatus,
  getPriceDetailsLikeHubspot,
  getLeafStatusLikeHubspot,
} = require("../../helpers/messages");
const {
  findAllCategories,
  getSubcategoryByIds,
} = require("../../dbQuery/cureCategory");
...
async function createOrganisationWithHubSpot(param, orgId, isNew) {
  try {
    const orgCreateHub = await hubspotCrm.createAndUpdateHubspotCompany(
      isNew,
      orgId,
      param
    );
...
async function getRemoveSubCategoryByOrganisation(
  orgId,
  categoryIds = [],
  subCategoryIds = []
) {
  try {
    const organisationDetails = await organisationCrm.getOrganisationByData({
      _id: orgId,
    });
    if (isEmpty(organisationDetails))
      return { err: true, msg: "Organisation Id not found" };
    ...
    console.log(
      `##oldOrganisationCategory##########`,
      oldOrganisationCategory,
      oldOrganisationSubCategory
    );
    const deleteCatIds = [];
    const deleteSubCatIds = [];
    for (let subCategory of oldOrganisationSubCategory) {
      const findSubCategory = find(
        subCategoryIds,
        (id) => id.toString() === subCategory.toString()
      );
    ...
    const findSiteBySubCategory = await siteOrm.findSitesByFields({
      organisationId: orgId,
      subcategory: { $in: deleteSubCatIds },
    });
    if (isEmpty(findSiteBySubCategory))
      return { err: false, msg: "No need to Change" };
    console.log(
      `Delete Category--${deleteCatIds}`,
      `Delete Subcategory--${deleteSubCatIds}`
    );
    for (let site of findSiteBySubCategory) {
      ...
      for (let id of oldSiteSubcategory) {
        ...
        if (!isEmpty(findDeletedId)) {
          await siteSubCategoryOrm.findAndDelete({
            siteId: site["_id"],
            subCategoryId: id,
          });
        }
      }
      await siteOrm.updateSite(
        {
          $pull: {
            subcategory: { $in: deleteSubCatIds },
            category: { $in: deleteCatIds },
          },
        },
        site["_id"]
      );
...
async function createOrganisation(data) {
  try {
    ...
    return {
      err: false,
      msg: create,
      val: successMessages.ORGANISATION_CREATED,
    };
  } catch (err) {
    return { err: true, msg: err };
  }
...
async function createOrgByAdmin(data) {
  try {
    if (!isEmpty(data.logo)) {
      let imageUrl = await imageUploadOrm.uploadBase64ToS3(
        data.logo,
        data.userId
      );
    ...
    if (!isEmpty(data.sourceOfFinding))
      await searchLinkOrm.getUpdateLink(data.sourceOfFinding, {
...
async function updateOneExistingOrganisation(orgId, data) {
  try {
    ...
    const provider = await providerOrm.filterProviderByfields({
      organization: mongoose.Types.ObjectId(orgId),
    });
    if (!isEmpty(provider))
      findOrg = { ...findOrg["_doc"], providerInfo: provider };
    if (!isEmpty(data.logo)) {
      let imageUrl = await imageUploadOrm.uploadBase64ToS3(
        data.logo,
        data.userId
    ...
    if (!isEmpty(data.subcategory)) {
      const checkSubCategory = await getRemoveSubCategoryByOrganisation(
        orgId,
        data.category,
        data.subcategory
      );
      if (!isEmpty(checkSubCategory) && checkSubCategory.err)
        return checkSubCategory;
    }
    ...
    if (!isEmpty(data["leadStatus"])) {
      if (
        ...
        const siteSubCategory = await siteOrm.fetchAllServicesByOrgId({
          organisationId: mongoose.Types.ObjectId(orgId),
        });
          ...
          for (let siteService of siteSubCategory) {
            ...
            for (let service of services) {
              if (!isEmpty(service["specialQualif"])) {
                const getSQ =
                  await specialQualificationOrm.dynamicSpecialQualification(
                    { _id: service["specialQualif"] },
                    { name: 1 }
                  );
                data.specialSQ = [
                  ...Object.keys(groupBy(getSQ, "name")),
                  ...data.specialSQ,
                ];
              }
              if (!isEmpty(service["specialQues"])) {
                data.sQFreeText.push(service["specialQues"]);
              }
              if (!isEmpty(service["price"])) {
                for (let pr of service["price"]) {
                  const valueLikeHubspot = getPriceDetailsLikeHubspot(pr);
                  if (!data.price.includes(valueLikeHubspot))
                    data.price.push(valueLikeHubspot);
                }
              }
              if (service["leaf"] && !roundUpLeaf.includes("yes")) {
                roundUpLeaf.push("yes");
              } else if (!service["leaf"] && !roundUpLeaf.includes("no"))
                roundUpLeaf.push("no");
            }
          }
          data.leaf = getLeafStatusLikeHubspot(roundUpLeaf);
        }
        const hubspotOrgCreation = await createOrganisationWithHubSpot(
          data,
          findOrg,
          isNew
        );
        if (!isEmpty(hubspotOrgCreation) && hubspotOrgCreation.err)
          return hubspotOrgCreation;
      }
    }
    return updateOrg;
  } catch (err) {
...
async function createDynamicOrganisation(data) {
  try {
    ...
    if (data.type === identity.ADMIN) {
      const getExistingOrganisationByName =
        await organisationCrm.getOrganisationByData({ name: data.name });
      if (!isEmpty(getExistingOrganisationByName))
        return { err: true, msg: "Organisation is Already exist" };
      ...
      const providerOrganisation = await organisationCrm.getOrganisationByData({
        providerId: data.providerId,
      });
      if (!isEmpty(providerOrganisation))
        return { err: true, msg: errorMessages.ORGANISATION_ALREADY_EXIST };
...
async function createCatSubcategory(organisation) {
  try {
    ...
    if (!isEmpty(organisation["subcategory"])) {
      const collectAllCategory = await getSubcategoryByIds(
...
async function getAdminOrganisationById(data, role) {
  try {
    ...
    if (role === identity.PROVIDER) {
      ...
      const provider = await providerOrm.getDynamicProvider(
        "userId",
        providerId,
        { organization: 1 }
      );
      getExistingOrganisation = await organisationCrm.getOrganisationByData({
        _id: provider[0].organization,
      });
      if (isEmpty(getExistingOrganisation))
        return { err: true, msg: "organisation Id not Exist" };
    ...
    getPendingStateOfDataApproval = await dataApproval.getOrgDataApproval({
      organisationId: getExistingOrganisation["_id"],
      status: status.PENDING,
    });
    if (role === identity.PROVIDER)
      approvalPendingData.approvalPending = !isEmpty(
        getPendingStateOfDataApproval
      );
    else {
      approvalPendingData.approvalPending = !isEmpty(
        getPendingStateOfDataApproval
      ...
      approvalPendingData.approvalData = !isEmpty(getPendingStateOfDataApproval)
        ? {
            ...getPendingStateOfDataApproval[0],
            cat: await createCatSubcategory(getPendingStateOfDataApproval[0]),
          }
        : {};
    }
    if (!isEmpty(organisation)) {
      ...
      details.push({
        organisation,
        cat: await createCatSubcategory(organisation),
        ...approvalPendingData,
      });
...
async function adminContactAllOrganisation(orgId) {
  try {
    ...
    if (isEmpty(organisation))
      return { err: true, msg: "organisation Id not Exist" };
    const getProviderByOrganisation = await providerOrm.getDynamicProvider(
      "organization",
      orgId,
      {
        organization: 1,
...
async function getOrganisationDetailsByOrgId(data) {
  try {
    const { orgId } = data;
    const orgData = [];
    const organisation = await organisationCrm.fetchAllInfoByOrganisationId([
      orgId,
    ]);
    console.log(organisation);
    const providerDetails = await providerOrm.filterProviderByfields(
      { organization: orgId, makeAccountPrimary: true },
      {
        name: 1,
        email: 1,
        contact: 1,
    ...
    const providerDetailsForSubProvider =
      await providerOrm.filterProviderByfields(
    ...
    if(orgData[0].claimStatus === false)
    {
...
async function deleteOrganisation(id) {
  try {
    ...
    const providerDelete = await providerOrm.filterProviderByfields({
      organization: id,
    });
...
async function bulkOrganisationData(data, subCategoryData, userId) {
  try {
    ...
    for (let objKeys of Object.keys(data)) {
      ...
      if (trimData === "Hubspot (ID) (Syncs in other direction)")
        items.hubspotId = data[objKeys];
      ...
      if (trimData === "Org Geospan")
        items.geospan = data[objKeys].split(";").map((val) => val.trim());
      if (trimData === "Record Status")
        items.leadStatus = leadSTatus(data[objKeys]);
      if (trimData === "Available Service Categories") {
        const dataValue = [];
        forEach(data[objKeys].split(";"), (cat) => {
          const subcategoryTrim = find(
            subCategoryData,
            (subId) =>
              subId.name.toLowerCase() === cat.trim().toLowerCase().toString()
          );
    ...
    const findOrg = await organisationCrm.getOrganisationByData({
      name: items.name,
      email: items.email,
    });
...
async function bulkSiteData(data, userId) {
  try {
    ...
    for (let siteKey of Object.keys(data)) {
      ...
      if (trimData === "Hubspot (ID) - Site Organization")
        hubspotId = data[siteKey].toString();
      ...
      if (trimData === "Home Visit")
        item.homeVisit = data[siteKey].toLowerCase() === "yes";
      if (trimData === "Is Additional Resource")
        item.additional = data[siteKey].toLowerCase() === "yes";
      if (trimData === "IS HQ (0/1)")
        item.HQ = data[siteKey].toLowerCase === "yes";
      if (trimData === "Service Radius") item.radius = data[siteKey];
      if (trimData === "Is Virtual")
        item.virtual = data[siteKey].toLowerCase() === "yes";
    }
    if (isEmpty(item.name)) return { err: true, msg: "Site Name not found" };
    const findOrganisation = await organisationCrm.getOrganisationByData({
      hubspotId,
    });
    if (isEmpty(findOrganisation))
      return { err: true, msg: "No Organisation Data found" };
    item.userId = userId;
    item.organisationId = findOrganisation["_id"];
    const findSite = await siteOrm.findSites({
...
async function bulkServiceData(data) {
  try {
    ...
    for (let serviceKey of Object.keys(data)) {
      ...
      if (trimData === "Associated Site Identifier (Site Name ID)")
        siteIdentifier = data[serviceKey];
      if (trimData === "Site Sub Category") {
        ...
        item.subCategoryId = !isEmpty(subcategoryTrim)
          ? subcategoryTrim[0]._id
          : "";
      }
      if (trimData === "Site Sub Category webpage")
        item.serviceWebpage = data[serviceKey];
      if (trimData === "Price") item.price = data[serviceKey].split(";");
      if (trimData === "Site Sub Category Description")
        item.serviceDescription = data[serviceKey];
      if (trimData === "Service Vanity Name")
        item.serviceName = data[serviceKey];
      if (trimData === "Leaf")
        item.leaf = data[serviceKey].toLowerCase() === "yes";
      ...
      if (trimData === "Additional Requirements")
        item.specialQues = data[serviceKey];
      if (trimData === "Special qualifications") {
        item.specialQualiFlag = true;
        const specialQ = map(data[serviceKey].split(";"), (qualificationName) =>
        ...
        item.specialQualif = isEmpty(qualificationIds)
          ? []
          : map(qualificationIds, (id) => id._id);
      }
    }
    if (isEmpty(hubspotId)) return { err: true, msg: "Hubspot Id not found" };
    if (isEmpty(siteIdentifier))
      return { err: true, msg: "siteIdentifier Id not found" };
    if (isEmpty(item.subCategoryId))
      return { err: true, msg: "subCategory Id not found" };
    const findOrg = await organisationCrm.fetchAllInfoByOrganisationIdByData({
      hubspotId,
    });
    if (isEmpty(findOrg)) return { err: true, msg: "Org in service not found" };
    const siteData = await find(
      findOrg,
      (data) => data.sitesInfo.siteIdentifier === siteIdentifier
    );
    if (isEmpty(siteData))
      return { err: true, msg: "Site service Data not found" };
    ...
    const serviceId = await siteSubCategoryOrm.createSiteSubcategory({
      ...item,
      siteId: site._id,
    });
...
async function geoCodingLocation(site) {
  try {
    ...
    if (!isEmpty(site["address"])) {
      ...
      request.get(
        `https://maps.googleapis.com/maps/api/geocode/json?address=${address},${site["city"]}&key=${key}`,
        {},
        async (err, data) => {
          if (err) {
            errFound = true;
...
async function siteCategoryAndSubCategory(poc = [], userId) {
  try {
    ...
    const siteSubCategory =
      await siteSubCategoryOrm.findSiteSubCategoryBySiteId(
        {},
        { subCategoryId: 1, siteId: 1 }
      );
    ...
    for (let site of allSite) {
      ...
      for (let siteSubCat of allSiteSubCat) {
        const findSubCategory = find(allSubCategory, {
          _id: siteSubCat.subCategoryId,
        });
        if (isEmpty(findSubCategory)) continue;
        if (!subCat.includes(siteSubCat.subCategoryId))
          subCat.push(siteSubCat.subCategoryId.toString());
    ...
    for (let org of Object.keys(organisationDetails)) {
      ...
      await organisationCrm.findIdAndUpdate(
        {
          category: uniq(details.categoryId),
          subcategory: uniq(details.subCategoryId),
        },
        org
      );
    }
    for (let user of poc) {
      if (!isEmpty(user.name) && !isEmpty(user.email)) {
        const createUser =
          await dataApprovalHelper.createOrgAndProviderBySubUser({
            ...user,
            userId,
            approvedStatus: status.APPROVED,
            organisationId: user.organisationId,
            sheet: true,
            acceptHippaDate: moment().utc(),
            hippa: true,
          });
        const pocId = createUser.id;
        const findSiteSubCategory =
          await siteSubCategoryOrm.findSiteSubcategory({ _id: user.serviceId });
        if (!isEmpty(findSiteSubCategory)) {
          await siteSubCategoryOrm.updateSiteCategory(user.serviceId, {
            $push: { poc: pocId },
          });
        }
      }
...
async function uploadOrganisation(file, userId) {
  console.log(file);
  const uploadXlsx = await imageUploadOrm.uploadExcelToParsingData(
    file[0].path
  );
  ...
  if (!isEmpty(type)) {
    for (let fileData of fileDataInJson) {
      if (type === "organisation") {
        const organisationDetails = await bulkOrganisationData(
          fileData,
          subCategoryData,
          userId
        );
