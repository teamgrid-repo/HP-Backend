...
} = require("../my_organisation/organisation.helper");
...
exports.register = async (req, res) => {
  try {
    ...
    if (!isEmpty(user))
      return errorResponse(req, res, errorMessages.USER_ALREADY_EXIST);

    const ghostUser = await pocAccount.findPocAccount({ email: param.email, isGhost:true });
    ...
    const data = new UsersModel({
      ...
      status: param.status,
      type: param.type,
      social_token: param.socialToken || "",
      fcm_token: param.fcmToken || null,
      jwt_auth_token: "",
      jwt_token_expired: "",
    });
    await data.save();
    if (param.role === "provider") {
      ...
      if (
        ...
        organisation.err &&
        organisation.msg !== errorMessages.HUBSPOT
      ) {
        await UsersModel.findByIdAndDelete(data._id);
        return errorResponse(req, res, organisation.msg);
      }
      //let hippaDate = {};
      //if (param.hippa) hippaDate.acceptHippaDate = moment().utc();
      //else hippaDate.acceptNonHippaDate = moment().utc();
      const newData = await newProvider({
        name: param.name,
        firstName: param.firstName,
        lastName: param.lastName,
        howYouHeard: param.howYouHeard,
        jobTitle: param.jobTitle,
        contact: param.phone,
        email: param.email,
        userId: data._id,
        organization: organisation.msg._id,
        hippa: param.hippa,
        approvedStatus: status.PENDING,
        identity: identity.MAIN_PROVIDER,
        acceptProviderTermsDate: moment().utc(),
      });
      if (
        !isEmpty(newData) &&
        newData.err &&
        newData.msg !== errorMessages.HUBSPOT
      ) {
        await UsersModel.findByIdAndDelete(data._id);
      ...
      const dataProfile = await createUserProfile({
        ...
        phone: param.phone,
        gender: param.gender,
        martialStatus: param.martialStatus,
        dob: param.dob,
        name: param.name,
      ...
      if (!isEmpty(dataProfile) && dataProfile.err) {
        ...
        await UsersModel.findByIdAndUpdate(data._id, {
    ...
    const originalData = await UsersModel.findById(data._id).populate(
      "profileId"
    );
    ...
    return successResponse(
      ...
      res,
      originalData,
      successMessages.REGISTRATION_DONE
    );
  } catch (error) {
...
exports.profile = async (req, res) => {
  try {
    const data = await UsersModel.findOne({ _id: req.user._id })
      .populate({ path: "assigningId", select: { email: 1, name: 1 } })
      .lean();
...
exports.getAll = async (req, res) => {
  try {
    ...
    const data = await UsersModel.find(condtionObj)
      .skip(skip)
      .limit(limit)
      .sort(sort)
      .lean();
...
exports.forgetPassword = async (req, res) => {
  try {
    ...
    if (!isEmpty(email)) {
      ...
      const template = await emailTemplate.geForgetPasswordTemplate(
        hashCode.code
      );
...
exports.getSavedUser = async (req, res) => {
  try {
    ...
    if (!loginUser)
      return errorResponse(req, res, errorMessages.USER_NOT_EXIST);
    let savedUser = param["provider"]
      ? loginUser["saved_provider"]
      : loginUser["saved_user"];
    let data;
    if (
      !isEmpty(savedUser) &&
      ((loginUser["role"] === "provider" && param["provider"]) ||
        loginUser["role"] === "user")
    ) {
...
exports.removeSavedUser = async (req, res) => {
  try {
    ...
    const data = await UsersModel.updateOne(
      { _id: req.user._id },
      { $pullAll: { saved_user: [savedUser], saved_provider: [savedUser] } }
    );
...
exports.uploadBase64Image = async (req, res) => {
  try {
    const param = { ...req.params, ...req.query, ...req.body };
    const upload = await userHelper.uploadImageByUserId({
      userId: req.user._id,
      image: param.image,
      filePath: req.files,
    });
...
module.exports.updateProviderInfo = async (req, res) => {
  try {
    ...
    const updateInfo = await userHelper.makeAccountPrimaryApprovalUsers(
      data,
      id
    );
...
module.exports.switchTab = async (req, res) => {
  try {
    ...
    if (isEmpty(getAssigningId))
      return errorResponse(req, res, "Assigning Email not Found");
    const user = await UsersModel.findOne({ _id: getAssigningId });
    const loginDetails = await userHelper.getLogin(
      { email: user["email"] },
      user
    );
