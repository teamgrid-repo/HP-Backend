...
const {
  getProviderById,
  getOneProvider,
  findProviderByOrganisation,
  getDynamicProviderWithoutPopulate,
  getProviderByName,
  updateProviderInfo,
  updateProviderInfoo
} = require("../../dbQuery/provider");
const {
  getOrganisationById,
  findIdAndUpdate,
} = require("../../dbQuery/organisation");
const {
  errorMessages,
  successMessages,
  identity,
...
async function login(param) {
  try {
    ...
    console.log("ghostUser------",ghostUser)
    ...
    if (!isEmpty(user) && user["freeze"]) {
      return { err: true, msg: "Account is freezed! Contact to Admin!!" };
    }
    if (param.type === signUpType.WEB) {
      ...
      param.type === signUpType.GOOGLE ||
      param.type === signUpType.FACEBOOK ||
      param.type === signUpType.APPLE
    ) {
      const signUptoken = user["social_token"];
      if (signUptoken !== param.socialToken || isEmpty(param.socialToken))
        return { err: true, msg: errorMessages.SIGNUP_TOKEN };
...
async function createNewAdditionalSubUser(param) {
  try {
    ...
    if (param.role !== identity.ADMIN) {
      ...
      if (isEmpty(findAccountOwner))
        return { err: true, msg: `Provider ${errorMessages.USER_NOT_EXIST}` };
      if (!isEmpty(findAccountOwner) && !userOwner.makeAccountPrimary)
        return { err: true, msg: "You Don't have primary account" };
    }
    /**
    ...
    const totalRegisteredUser = await findProviderByOrganisation(
      organisationDetails._id
    );
    console.log(organisationDetails, totalRegisteredUser);
    if (param.role !== identity.ADMIN) {
      const findNameAndEmail = await UsersModel.findOne({
        email: param.email,
      }).lean();
      if (!isEmpty(findNameAndEmail))
        return { err: true, msg: "Email of this account is already Exist" };

      const ghostUser = await pocAccount.findPocAccount({ email: param.email });
      if (!isEmpty(ghostUser)) return { err: true, msg: "Email of this account is already Exist" }

      if (
        size(totalRegisteredUser) ===
        organisationDetails["totalAssigned"] + 1
      ) {
        return {
          err: true,
          msg: "Can't added more subUser.Please contact to admin to increase limit",
        };
      }
      const createdNewReq =
        await dataApprovalSubUser.createDataApprovalForSubUser({
          ...param,
          requestBy: userId,
    ...
    const newSubUserCreated =
      await dataApprovalSubUser.createOrgAndProviderBySubUser(param, false);
    if (!isEmpty(newSubUserCreated) && newSubUserCreated.err)
      return newSubUserCreated;
    return {
      err: false,
      msg: successMessages.PROVIDER_CREATED,
    };
  } catch (err) {
    return {
      err: true,
...
async function getAdditionalSubUser(data, role) {
  try {
    ...
    let providerData = await getDynamicProviderWithoutPopulate(
      "organization",
      [orgId],
      {
        createdAt: 0,
    ...
    const providerInfo = [];
    const getSubUser = await dataApprovalOrm.getDataApprovalSubUser(
      { requestBy: userId, status: status.PENDING },
      {
        createdAt: 0,
        updatedAt: 0,
        __v: 0,
    ...
    for (let accountOwner of providerData) {
      const getAllocation =
        await siteSubCategoryHelper.getAllSiteSubcategoryByUserId([
          accountOwner.userId,
        ]);
      const getSubUserPending = filter(
        getSubUser,
        (p) =>
          p.method !== method.CREATE &&
          !isEmpty(p.subUserId) &&
          p.subUserId._id.toString() === accountOwner["userId"].toString()
      );
      if (!isEmpty(getAllocation) && getAllocation.err) {
        return getAllocation;
      }
      providerInfo.push({
        ...accountOwner,
        totalAssigned: organisation["totalAssigned"]
          ? organisation["totalAssigned"]
          : 10,
...
async function updateSubUserProvider(data, value) {
  try {
    ...
    sendMail(data.currUserEmail, "Update Request", update);
    return {
      err: false,
      msg: successMessages.APPROVAL_CREATED,
      val: successMessages.APPROVAL_CREATED,
    };
  } catch (err) {
    return {
      err: true,
...
async function uploadImageByUserId(data) {
  try {
    ...
    if (isEmpty(image) && isEmpty(filePath))
      return { err: true, msg: "Image Buffer not found" };
    const users = await UsersModel.findOne({ _id: userId });
    if (isEmpty(userId) || isEmpty(users))
  ...
  try {
    ...
    if (Object.keys(data).includes("makeAccountPrimary")) {
      ...
      if (data.EmailMessage)
        sendNotificationToUser({ isEmail: true, email: provider["email"] });
      if (data.textMessage && !isEmpty(provider["contact"]))
        sendNotificationToUser({ isText: true, contact: provider["contact"] });
