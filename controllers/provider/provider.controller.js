...
module.exports.createProvider = async (req, res) => {
  ...
  try {
    ...
    return successResponse(
      ...
      res,
      value.msg,
      successMessages.PROVIDER_UPDATED
    );
  } catch (err) {
...
module.exports.getProviderByUserId = async (req, res) => {
  try {
    ...
    if (!isEmpty(userId)) {
      ...
      const siteInfoByProvider =
        await providerSiteHelper.getSiteWithSubCategoryInfoByUserId(
          getProvider
        );
      ...
      let provider = {
        ...getProvider["_doc"],
        searchResults: getProvider["organization"].searchResults,
        totalAssigned: getProvider["organization"].totalAssigned || 10,
        siteInfo: siteInfoByProvider.msg,
...
module.exports.saveClients = async (req, res) => {
  try {
    const param = { ...req.body, ...req.params, ...req.query };
    if (isEmpty(param.clientId))
      return errorResponse(req, res, errorMessages.INVALID_PARAMS);
    param.userId = req.user._id;
    const getProviderInfo = await ProviderOrm.filterProvider({
      userId: req.user._id,
    });
    if (isEmpty(getProviderInfo))
      return errorResponse(req, res, "Provider Details Not found");
    const clientProfile = await userProfileOrm.getUserProfileById(
      param.clientId
    );
    if (isEmpty(clientProfile))
      return errorResponse(req, res, "Client Id is not valid");
    const provider = getProviderInfo[0];
    const saveClientData = provider.saveClientUserId || [];
    const checkClientAlreadySaved = await ProviderOrm.filterProvider({
      saveClientUserId: { $in: param.clientId },
      _id: provider._id,
    });
...
module.exports.deleteSaveClient = async (req, res) => {
  try {
    ...
    const clientProfile = await ProviderOrm.filterProvider({
      saveClientUserId: { $in: clientId },
      _id: provider._id,
    });
    if (isEmpty(clientProfile))
      return errorResponse(req, res, "Client Id is not valid");
...
module.exports.updateAccountApproval = async (req, res) => {
  try {
    ...
    const userId = req.user._id;
    const updateAccountApproval = await providerHelper.updateAccountApproval(
      data,
      userId
    );
...
module.exports.shiftProviderOrganisation = async (req, res) => {
  try {
    ...
    const userId = req.user._id;
    const provider = await providerHelper.shiftProviderOrganisation(
      param,
      userId
    );
    ...
    return successResponse(
      ...
      res,
      provider.msg,
      successMessages.SHIFT_PROVIDER
    );
  } catch (err) {
    return errorResponse(req, res, err);
  }
    ...
    if (role === "admin" || role === "provider") {
      ...
      if (findProvider) {
        ...
        const approvedSubUserTemplate = content([
          "You are invited to set up a login to the Her PLAN directory! "+ `<a href="https://staging.coppervine.dev/login" target="_blank">Click here to login </a>` + ". Here is your temporary password. " +
            code +
