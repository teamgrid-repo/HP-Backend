...
async function searchesSavedClient(dataValues) {
  try {
    ...
    const getProvider = await providerOrm.getDynamicProvider(
      "userId",
      [userId],
      { saveClientUserId: 1 }
    );
    if (isEmpty(getProvider))
      return { err: true, msg: "Provider Details not found" };
    ...
    if (!isEmpty(data)) {
      ...
      data = await userProfileOrm.getUserProfileOfArray(
        Object.keys(groupByUserId)
      );
    }
...
async function updateAccountApproval(data, currUserId) {
  try {
    ...
    if (
      data.approvedStatus === status.APPROVED ||
      data.approvedStatus === status.CANCELLED
    ) {
      if (!isEmpty(data["newOrgId"])) {
        const orgId = data["newOrgId"];
        const findOrg = await organisationOrm.getOrganisationById(orgId);
        if (isEmpty(findOrg))
          return { err: true, msg: "Selected organisation not found" };
        const oldOrgProvider = await providerOrm.filterProvider({
        ...
        for (let provider of oldOrgProvider) {
          ...
          await providerOrm.dynamicUpdateProvider(
            "_id",
            provider["_id"],
            updatedData
          );
...
async function shiftProviderOrganisation(data) {
  try {
    ...
    if (isEmpty(provider))
      return { err: true, msg: "Email not Found! Please Register" };
    ...
    for (let poc of findPoc) {
      await siteSubcategoryOrm.updateSiteCategory(poc["_id"], {
        $pull: { poc: user.userId._id },
      });
    }
    ...
    const hippaData = {
      hippa: map(filter(getProvider, { hippa: true }), (provider) => {
        let findOrg =
          find(
            getAllOrganisation,
            (org) => org._id.toString() === provider.organization.toString()
          ) || {};
        ...
        return {
          ...
          ...pick(findOrg, ["orgName", "address", "city", "state"]),
        };
      }),
